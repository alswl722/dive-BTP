...
> 상태: **DeepSeek 실 호출 활성화 · 배치 실행 완료** — `axis8_llm_cache.json` fixture에 판정 결과 반영됨(§7-1, §8). 캐시에 없는 신규 조합만 `businessFit.llm` mock 플래그로 표기.
...
- ✅ **배치 실행 완료** — `axis8_llm_cache.json` fixture에 판정 결과 반영됨(§7-1 배포 흐름 참고). `company_view.py:_compute_mock_flags()`는 캐시에 없는 신규 조합만 `businessFit.llm` mock 플래그로 표기.
...
- dedup 캐시 자동 동작
- `company_view.py:build_business_fit()`이 whitelist/LLM 결과를 `BusinessFit`으로 조립하는 배선 완료

---

## 9. 다음 단계 — 캐시 확장

- 본선 데이터 투입 시 `scripts/run_axis8_llm_batch.py` 재실행해 신규 조합 커버
- 배치 로깅으로 실측 비용·캐시 히트율 수집 → 본 문서 §11(비용 실측) 재산정
- 재실행 후 fixture(`axis8_llm_cache.json`) 갱신·커밋 필요(§7-1)
...
DeepSeek + gpt-4o 병렬 판정 → 두 모델 이견 시 human review flag. 5000기업 20% 시나리오 비용: DeepSeek $0.52 + gpt-4o $12.3 = **$12.82**(크레딧 이내). 담당자 화면에 "AI 이견" 배지로 발표 어필 가능. 우선순위: gold label 20~30건 확장 시점에 실효성 판단.
