...
import { CompositeMiniBars } from "@/components/companies/composite-mini-bars";
      ...
      <Table>
        <THead>
          <TR>
            ...
            {/* 종합점수를 이루는 7축(재무4·기술2·정합성) 세부 — 상세 개요 그래프의 표 셀 버전 */}
            <TH className="w-[248px]">축별 점수</TH>
            <TH className="w-[184px]">보유 인증</TH>
          </TR>
        </THead>
        <TBody>
          {pageItems.map((c) => {
            ...
            return (
              ...
              >
                ...
                <TD>
                  {/* 종합점수 7축 세부 — 최저축은 warn 색으로 강조 */}
                  <CompositeMiniBars cs={c.compositeScore} />
                </TD>
                <TD>
                  {/* 보유 인증 6종 — 3×2 그리드. 보유는 초록, 미보유는 회색. 상세 개요와 동일 데이터 */}
                  <div className="grid w-[168px] grid-cols-3 gap-1">
                    {Object.entries(c.certifications).map(([k, has]) => (
                      <Badge
                        key={k}
                        variant={has ? "good" : "secondary"}
                        className={cn("justify-center px-1 py-0 text-[10px]", !has && "opacity-50")}
                      >
                        {k}
                      </Badge>
                    ))}
