...
import { DEFAULT_AXIS_WEIGHTS, DEFAULT_GROUP_WEIGHTS, DEFAULT_TECH_WEIGHTS, TECH_AXES, isCustomWeights, type TechAxis } from "@/lib/scoring";
...
const TECH_AXIS_BAR_COLOR: Record<TechAxis, string> = {
  "R&D특허": "bg-axis-profit",
  NTIS: "bg-axis-efficiency",
};
...
export function WeightPopover({
  ...
  useEffect(() => {
    if (open) {
      ...
      setDraftTech(techWeights);
    }
  }, [open, groupWeights, weights, techWeights]);
  ...
  const techTotal = Math.round(TECH_AXES.reduce((s, a) => s + draftTech[a], 0));
  const isCustom = isCustomWeights(groupWeights, weights, techWeights); // 버튼 표시는 '적용된' 값 기준
        ...
        <>
          ...
          <div className="absolute right-0 top-full z-20 mt-2 w-[340px] space-y-5 rounded-xl bg-card p-4 shadow-modal">
            ...
            <p className="text-[11px] text-muted-foreground">
              그룹 비율(재무/기술/정합성)과 재무·기술 세부축 비율을 각각 조정할 수 있습니다. 최저축 캡(최저축+15점)은
            ...
            {/* 기술 세부축(R&D특허 / NTIS) — 기술 그룹 내부 배분. 기술 점수 컬럼·종합점수에 함께 반영 */}
            <div className="border-t pt-4">
              <p className="mb-2 text-[11.5px] font-bold text-muted-foreground">기술 세부축 비율 (기술 그룹 내부 배분)</p>
              <div className="flex h-2 w-full overflow-hidden rounded-full">
                {TECH_AXES.map((axis) => (
                  <div key={axis} className={TECH_AXIS_BAR_COLOR[axis]} style={{ width: `${draftTech[axis]}%` }} />
                ))}
              </div>
              <div className="mt-3 space-y-3">
                {TECH_AXES.map((axis) => (
                  <div key={axis} className="space-y-1">
                    <div className="flex items-baseline justify-between text-[12px]">
                      <span>{axis === "R&D특허" ? "R&D · 특허" : "NTIS (정부 R&D)"}</span>
                      <span className="tabular-nums font-bold">{Math.round(draftTech[axis])}%</span>
                    </div>
                    <input
                      type="range"
                      min={0}
                      max={100}
                      value={Math.round(draftTech[axis])}
                      onChange={(e) => setDraftTech(rebalance(draftTech, TECH_AXES, axis, Number(e.target.value)))}
                      className="w-full accent-primary"
                    />
                  </div>
                ))}
              </div>
              <p className={cn("mt-2 text-right text-[11px] font-medium tabular-nums", Math.abs(techTotal - 100) <= 1 ? "text-good" : "text-bad")}>
                합계 {techTotal}%
              </p>
            </div>
