...
export function EmploymentTab({ company, latestYear }: { company: Company; latestYear: number }) {
  ...
  return (
    <div className="space-y-5">
      ...
      <Selectable id="emp-scale"><ScaleSection company={company} /></Selectable>

      {/* 2. 처우 — 서류상 우수 고용 주장을 실측 급여로 감사 */}
      <Selectable id="emp-salary"><SalarySection company={company} /></Selectable>

      {/* 3. 인력 생산성 — 성장·수익축과 독립적으로 자본 효율 진단 */}
      <Selectable id="emp-productivity"><ProductivitySection company={company} /></Selectable>

      {/* 4. 인력 안정성 — 국민연금 3종 기반 이직·회전 */}
      <Selectable id="emp-stability"><StabilitySection company={company} /></Selectable>
    </div>
  );
}
