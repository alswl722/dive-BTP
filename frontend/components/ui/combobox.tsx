...
/** 검색 가능한 다중 선택 콤보박스 — 체크박스로 여러 값을 동시에 고르는 필터용.
 *  선택 항목이 있으면 버튼에 "N개 선택"으로 요약 표시한다. */
            ...
            {value.length > 0 && (
              <div className="border-t p-1.5">
                <button
                  type="button"
                  onClick={() => onChange([])}
                  className="w-full rounded-md px-2.5 py-1 text-center text-[12px] text-muted-foreground hover:bg-muted hover:text-foreground"
                >
                  전체 해제
                </button>
              </div>
